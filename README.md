...
| **Asset Type** | Insurance Policy |
| **Control Record** | Insurance Policy Administrative Plan |
...
| POST | `/v1/insurance-policy-administrative-plan/initiate` | Initiate |
| GET | `/v1/insurance-policy-administrative-plan` | Retrieve (list) |
| GET | `/v1/insurance-policy-administrative-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/insurance-policy-administrative-plan/{crId}/update` | Update |
| PUT | `/v1/insurance-policy-administrative-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/insurance-policy-administrative-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-corporate-insurance:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-corporate-insurance ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-corporate-insurance` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
